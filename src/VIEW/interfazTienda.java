...
public class interfazTienda {
	...
	public static void main(String[] args,usuario usuario) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					interfazTienda window = new interfazTienda(usuario);
	 ...
	 */
	public interfazTienda(usuario usuario) {
		initialize(usuario);		
	}
	/**
	...
	private void initialize(usuario usuario) {
		...
		lblNewLabel.setBounds(327, 135, 412, 31);
		...
		priceLabel.setBounds(415, 630, 117, 20);
		...
		btnNewButton.setBounds(324, 565, 129, 23);
		frmAmadeusTienda.getContentPane().add(btnNewButton);
		btnNewButton.setVisible(false);

		JLabel storeWelcome = new JLabel("Bienvenido a la tienda - "+usuario.getUsuario());
		...
		buy.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if(sel==null) {
					...
					interfazCompra.main(new String[0],game,usuario);
				}
			}
		});
		buy.setBounds(551, 627, 89, 23);
		frmAmadeusTienda.getContentPane().add(buy);
